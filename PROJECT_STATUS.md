...
M1 notebook defaults to **CPU / fp32** (`USE_GPU = False`). Flip it only after pinning
`CUDA_VISIBLE_DEVICES` to an idle device — the notebook asserts this rather than
silently taking physical GPU 0.
